...
template <typename T> class IndexedMatrix {
    ...
    size_t         rows, cols;  // shape of the matrix
    const size_t * index;       // pointer to row indices
    size_t         indexCount;  // number of selected rows

  public:
    // Constructor
    __host__ __device__ IndexedMatrix(const T * d, size_t r, size_t c, const size_t * idx, size_t n) :
        data(d),
        rows(r),
        cols(c),
        index(idx),
    ...
    class Iterator {
        ...
        size_t                linearPos;  // flattened position in selected rows
      ...
      public:
        ...
        __host__ __device__ Iterator(const IndexedMatrix * p, size_t pos) : parent(p), linearPos(pos) {}

        // Dereference
        __host__ __device__ reference operator*() const {
            size_t rowIndex = parent->index[linearPos / parent->cols];
            size_t colIndex = linearPos % parent->cols;
    ...
    __host__ __device__ T & operator()(size_t i, size_t j) {
        size_t rowIndex = index[i];
        return data[rowIndex * cols + j];
    }

    const __host__ __device__ T & operator()(size_t i, size_t j) const {
        size_t rowIndex = index[i];
        return data[rowIndex * cols + j];
    }

    // Shape getters
    __host__ __device__ size_t numRows() const { return indexCount; }

    __host__ __device__ size_t numCols() const { return cols; }
};
...
template <class ProblemShape,
          ...
          class Alpha,
          class Beta>
__global__ void idx_neg_dot_kernel(ProblemShape shape_BTVC,
                                   CtaTiler     cta_tiler,
                                   TA const *   embd,
                                   AStride      stride_A,
...
template <typename Element,
          ...
          class Alpha,
          class Beta>
void idx_neg_dot(T_EMBD &       embd,
                 T_CLASSIFIER & classifier,
                 T_INDS &       Inds,
                 T_OUTPUT &     indexNegDot,
                 problemShape & problemShape_BTVC,
                 Alpha          alpha,
                 Beta           beta) {
    // Define CTA size.
    ...
    const dim3 grid_dims = static_cast<unsigned int>(cute::size(cute::ceil_div(cute::size<0>(problemShape_BTVC), bBT)));

    idx_neg_dot_kernel<<<grid_dims, block_dims>>>(problemShape_BTVC,
                                                  cta_tiler,
                                                  embd.device_data(),
                                                  embd.stride(0),
                                                  smem_layout_A,
                                                  ...
                                                  thread_layout_B,
                                                  Inds.device_data(),
                                                  indexNegDot.device_data(),
                                                  indexNegDot.stride(0),
                                                  smem_layout_C,
                                                  thread_layout_C,
                                                  tiled_mma,
                                                  alpha,
...
int main() {
    ...
    cutlass::HostTensor<Element, cutlass::layout::RowMajor> indexNegDot({ cute::size<0>(BT_V_C_SHAPE), cute::_1{} });
    cutlass::HostTensor<size_t, cutlass::layout::RowMajor>  Inds({ cute::size<0>(BT_V_C_SHAPE), cute::_1{} });

    for (int i = 0; i < cute::size<0>(BT_V_C_SHAPE); i++) {
        for (int j = 0; j < cute::size<2>(BT_V_C_SHAPE); j++) {
            embd.at({ i, j }) = 0.00001 * ((i * C) + j);
        }
        Inds.at({ i, cute::_0{} }) = i;  //(cute::size<0>(BT_V_C_SHAPE) - 1) - i;
    }
    ...
    idx_neg_dot<Element>(embd, classifier, Inds, indexNegDot, BT_V_C_SHAPE, alpha, beta);
