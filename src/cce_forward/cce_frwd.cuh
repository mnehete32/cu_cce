...
__global__ void cce_frwd_kernel(ProblemShape   shape_BTVC,
    ...
    auto tBgB               = thread_gmem_copy_B.partition_S(gB);  // (CPY, CPY_V, CPY_C, c)
    auto tBsB               = thread_gmem_copy_B.partition_D(sB);  // (CPY, CPY_V, CPY_C)
    auto tBrB               = cute::make_fragment_like(tBsB);      // (CPY, CPY_V, CPY_C)

    // Partition via MMA.
    auto thread_mma = tiled_mma.get_slice(threadIdx.x);

    auto tCsA     = thread_mma.partition_A(sA);  // (MMA, MMA_BT, MMA_C)
    auto tCsB     = thread_mma.partition_B(sB);  // (MMA, MMA_V, MMA_C)
    ...
    auto tCrB = cute::make_fragment_like(tCsB);  // (MMA, MMA_V, MMA_C)
    cute::clear(tCrC);
...
void cce_fwd(float * lse, const Element * embd, const Element * classifier, int BT, int V, int C) {
